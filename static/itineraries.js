...
function renderItineraryCards() {
  ...
  if (hasSearch) {
    ...
    items = items.filter(i => {
      ...
      return title.includes(q) || ref.includes(q) || psgName.includes(q) || psgEmail.includes(q) || billName.includes(q) || billAcct.includes(q);
    });
  }
    ...
    return `<div class="itin-card" onclick="openItinerary('${it.id}')">
      ...
      <div class="itin-card-body">
        <div class="itin-card-header">
          ...
          <span class="status-badge ${it.status}">${(it.status || '').replace('_', ' ')}</span>
        </div>
...
function renderDetailView() {
  ...
  document.getElementById('detailSubtitle').innerHTML = `
    <span class="status-badge ${it.status}">${getStatusIcon(it.status)} ${(it.status || '').replace('_', ' ')}</span>
...
function renderActions() {
  ...
  if (it.status === 'draft') {
    ...
    html += `<button class="btn-action small success" onclick="confirmItinerary()">🎫 Confirm</button>`;
...
function renderProgressBar() {
  ...
  const statusOrder = { 'draft': 0, 'approved': 1, 'on_hold': 2, 'confirmed': 2, 'reverted': 1 };
  const current = statusOrder[it.status] || 0;
  let html = '<div class="progress-steps">';
  const labels = [{ icon: '📝', name: 'Draft' }, { icon: '✅', name: 'Approved' }, { icon: '🎫', name: 'Finalized' }];
  labels.forEach((s, i) => {
...
function renderPassengers() {
  ...
  container.innerHTML = pax.map((p, i) => {
    ...
    const detail = [p.email, p.phone, p.age ? p.age + 'y' : '', p.gender].filter(Boolean).join(' • ');
    const isLinked = p.passenger_id || p.id;
    return `<div class="pax-item">
      <div class="pax-info">
        <div class="pax-avatar">${name.charAt(0).toUpperCase()}</div>
        <div>
          <div class="pax-name">${name} ${isLinked ? '<span class="fare-tag" style="font-size:0.7rem;background:rgba(16,185,129,0.1);color:var(--success);">📎 DB Linked</span>' : '<span class="fare-tag" style="font-size:0.7rem;">📝 Manual</span>'}</div>
          <div class="pax-detail">${detail || 'No contact info'}</div>
        </div>
      </div>
      <div style="display:flex;gap:0.5rem;align-items:center;">
        ${!isFinal ? `<button class="pax-remove" onclick="removePassenger(${i})" title="Remove">✕</button>` : ''}
      </div>
    </div>`;
  }).join('');
}
...
function renderBilling() {
  ...
  document.getElementById('billingContainer').innerHTML = `<div class="billing-grid">
    ...
    <div class="billing-item"><div class="label">GST</div><div class="value">${it.bill_to_gst || '-'}</div></div>
...
async function holdItinerary() {
  try {
    const r = await fetch('/api/v2/itineraries/' + currentItinerary.id + '/hold', { method: 'POST' });
    if (r.ok) { const d = await r.json(); currentItinerary = d.itinerary; renderDetailView(); showToast('Itinerary on hold', 'success'); }
...
async function confirmItinerary() {
  try {
    ...
    if (r.ok) { const d = await r.json(); currentItinerary = d.itinerary; renderDetailView(); showToast('Itinerary confirmed!', 'success'); }
    else showToast('Failed', 'error');
  } catch (e) { showToast('Error', 'error'); }
}
...
// ==================== MODALS ====================
function showModal(id) { document.getElementById('modalOverlay').classList.add('show'); document.getElementById(id).classList.add('show'); }
function closeModal() { document.getElementById('modalOverlay').classList.remove('show'); document.querySelectorAll('.modal').forEach(m => m.classList.remove('show')); }
...
function showLinkPassengerModal() {
...
document.addEventListener('DOMContentLoaded', async () => {
  initializeSidebar();
  await checkAuth();
  await Promise.all([loadItineraries(), loadDBPassengers(), loadDBBillingAccounts()]);
});
